...
public class EmployeeListServiceImpl implements EmployeeListService {
	...
	public void getEmployeeListUsingPagination(HttpServletRequest request, Model model) {
		...
		// 파라미터 column이 전달되지 않는 경우 column=EMPOYEE_ID로 처리한다.
		...
		model.addAttribute("beginNumber", totalRecord - (page - 1) * recordPerPage);
		switch(order) {
		case "ASC" : model.addAttribute("order", "DESC"); break;	// 현재 ASC 정렬이므로 다음 정렬은 DESC이라고 Jsp에 알려준다.
		case "DESC" : model.addAttribute("order", "ASC"); break;	// 현재 DESC 정렬이므로 다음 정렬은 ADC라고 Jsp에 알려준다.
		}
